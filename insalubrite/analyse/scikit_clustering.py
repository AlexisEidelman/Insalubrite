...
tab_en_attendant = X.select_dtypes(['float', 'bool'])
for name, col in tab_en_attendant.select_dtypes(['bool']).iteritems():
    print(name)
    tab_en_attendant[name] = 1*tab_en_attendant[name]
# on supprime 188 lignes qui sont pas matché
Y = X['est_insalubre']
del tab_en_attendant['est_insalubre']

data = scale(tab_en_attendant)
...
reduced_data = PCA(n_components=2).fit_transform(tab_afc)
...
kmeans.fit(scale(tab_afc))
classes = kmeans.predict(tab_afc)
